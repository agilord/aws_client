...
void main() {
  ...
  test('RestJsonHttpPayloadTraitsWithBlobAcceptsAllContentTypes', () async {
    final client = MockClient((request) async {
      expect(request.body, equalsJson(r'''This is definitely a jpeg'''));
      expect(request.headers['X-Foo'], 'Foo');
    ...
    await service.httpPayloadTraits(
      blob: Uint8List.fromList('This is definitely a jpeg'.codeUnits),
      foo: "Foo",
    );
  }, skip: r'''Auto-recorded: Suite 2 vector fails today''');

  test('RestJsonHttpPayloadTraitsWithBlobAcceptsNoContentType', () async {
    final client = MockClient((request) async {
      expect(request.body, equalsJson(r'''This is definitely a jpeg'''));
      expect(request.headers['X-Foo'], 'Foo');
      expect(request.url, equalsPathAndQuery('/HttpPayloadTraits'));
  ...
  test('RestJsonHttpPayloadTraitsWithBlobAcceptsAllAccepts', () async {
    final client = MockClient((request) async {
      expect(request.body, equalsJson(r'''This is definitely a jpeg'''));
      expect(request.headers['X-Foo'], 'Foo');
      expect(request.headers['Accept'], 'image/jpeg');
      expect(request.url, equalsPathAndQuery('/HttpPayloadTraits'));
      expect(request.method, equalsIgnoringCase('POST'));
      return Response('{}', 200);
    });
