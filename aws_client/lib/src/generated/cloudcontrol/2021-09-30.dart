...
class CloudControlApi {
  ...
  /// </note> <pre><code> &lt;p&gt;Specify the desired state as one of the
  ...
  Future<CreateResourceOutput> createResource({
    ...
    ArgumentError.checkNotNull(desiredState, 'desiredState');
  ...
  Future<UpdateResourceOutput> updateResource({
    ...
    ArgumentError.checkNotNull(identifier, 'identifier');
    ArgumentError.checkNotNull(patchDocument, 'patchDocument');
    ArgumentError.checkNotNull(typeName, 'typeName');
    final headers = <String, String>{
      'Content-Type': 'application/x-amz-json-1.0',
...
class ResourceRequestStatusFilter {
  ...
  factory ResourceRequestStatusFilter.fromJson(Map<String, dynamic> json) {
    return ResourceRequestStatusFilter(
      operationStatuses: (json['OperationStatuses'] as List?)
          ?.whereNotNull()
          .map((e) => (e as String).toOperationStatus())
          .toList(),
      operations: (json['Operations'] as List?)
          ?.whereNotNull()
          .map((e) => (e as String).toOperation())
          .toList(),
    );
  }
