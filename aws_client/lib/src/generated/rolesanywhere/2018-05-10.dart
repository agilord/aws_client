...
/// to access AWS resources. Using IAM Roles Anywhere will eliminate the need to
/// manage long term credentials for workloads running outside of AWS.
///
/// To use IAM Roles Anywhere customer workloads will need to use X.509
/// certificates issued by their Certificate Authority (CA) . The Certificate
/// Authority (CA) needs to be registered with IAM Roles Anywhere as a trust
/// anchor to establish trust between customer PKI and IAM Roles Anywhere.
/// Customers who do not manage their own PKI system can use AWS Certificate
/// Manager Private Certificate Authority (ACM PCA) to create a Certificate
...
/// programmatically. For general information about IAM Roles Anywhere see <a
/// href="https://docs.aws.amazon.com/">https://docs.aws.amazon.com/</a>
class IamRolesAnywhere {
  ...
  Future<ProfileDetailResponse> createProfile({
    ...
    ArgumentError.checkNotNull(roleArns, 'roleArns');
    _s.validateNumRange(
      'durationSeconds',
      durationSeconds,
      900,
  ...
  /// your certificate authority (CA) by configuring a trust anchor. A Trust
  /// Anchor is defined either as a reference to a AWS Certificate Manager
  /// Private Certificate Authority (ACM PCA), or by uploading a Certificate
  /// Authority (CA) certificate. Your AWS workloads can authenticate with the
  /// trust anchor using certificates issued by the trusted Certificate
  /// Authority (CA) in exchange for temporary AWS credentials.
  ///
  ...
  Future<CrlDetailResponse> deleteCrl({
    ...
    ArgumentError.checkNotNull(crlId, 'crlId');
    final response = await _protocol.send(
      payload: null,
      method: 'DELETE',
  ...
  Future<TrustAnchorDetailResponse> deleteTrustAnchor({
    ...
    ArgumentError.checkNotNull(trustAnchorId, 'trustAnchorId');
    final response = await _protocol.send(
      payload: null,
      method: 'DELETE',
  ...
  Future<CrlDetailResponse> importCrl({
    ...
    ArgumentError.checkNotNull(crlData, 'crlData');
    ArgumentError.checkNotNull(name, 'name');
    ArgumentError.checkNotNull(trustAnchorArn, 'trustAnchorArn');
    final $payload = <String, dynamic>{
  ...
  /// previous operation did not show all results. To get the next results, call
  /// the operation again with this value.
  ///
  /// Parameter [pageSize] :
  ...
  /// and your certificate authority (CA) by configuring a trust anchor. A Trust
  /// Anchor is defined either as a reference to a AWS Certificate Manager
  /// Private Certificate Authority (ACM PCA), or by uploading a Certificate
  /// Authority (CA) certificate. Your AWS workloads can authenticate with the
  /// trust anchor using certificates issued by the trusted Certificate
...
class ListCrlsResponse {
  ...
  /// operation did not show all results. To get the next results, call the
  /// operation again with this value.
...
class SourceData {
  ...
  /// Authority specified by this ARN is used in trust validation for <a
  /// href="https://docs.aws.amazon.com/rolesanywhere/latest/APIReference/API_CreateSession.html">CreateSession</a>
